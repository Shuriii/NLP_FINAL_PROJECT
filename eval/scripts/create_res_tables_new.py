...
def summarize_results(datasets):
    ...
    for dataset in datasets:
        ...
        for df_rows, baseline_id, label in [(llama_rows, 13, "llama"), (gemma_rows, 1, "gemma")]:
            ...
            df["family"] = label
            df["dataset"] = dataset
            df["super_category"] = "all"

            columns_to_keep = ['model', 'configuration', 'position', 'technique', 'size', 'continuous', score_col, 'total', 'correct', 'accuracy_percent', 'average_f1', 'average_f1_percent', 'total_evaluated', '% improvement', 'family', 'dataset', 'super_category']
            df = df[[col for col in columns_to_keep if col in df.columns]]

            # Format numeric values to 3 decimal places
            for col in ["average_f1", "accuracy", "accuracy_percent", "average_f1_percent", "% improvement"]:
            ...
            general_path = os.path.join(output_folder, f"{dataset}_{label}_all_{now}.csv")
            df.to_csv(general_path, index=False)

            if dataset in ["mmlu", "drop", "musique"] and 'per_super_category' in df_rows[0]:
                ...
                for supercat in supercats:
                    ...
                    if supercat_rows:
                        ...
                        # Format numeric values to 3 decimal places
                        for col in ["average_f1", "accuracy", "accuracy_percent", "average_f1_percent", "% improvement"]:
                            if col in supercat_df.columns:
                                supercat_df[col] = supercat_df[col].map(lambda x: f"{x:.3f}" if pd.notnull(x) else "")

                        # Ensure column order
                        supercat_df = supercat_df[[col for col in columns_to_keep if col in supercat_df.columns]]
